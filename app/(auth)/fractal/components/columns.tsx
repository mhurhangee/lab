...
export const columnsFractals: ColumnDef<ContextDB>[] = [
  {
    accessorKey: 'name',
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="font-medium">
          <Link href={`/fractal/${row.original.id}`}>{name as string}</Link>
        </div>
      )
    },
  },
    ...
    cell: ({ row }) => {
      return (
        <div className="max-w-[300px] truncate">
          <Link href={`/fractal/${row.original.id}`}>Fractal</Link>
        </div>
      )
    },
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div>
          <Link href={`/fractal/${row.original.id}`}>{formatted}</Link>
        </div>
      )
    },
  },
